...
| `hopCountAlpha` | `0.0` (off) | weight | `HopCountAlpha` | Mechanism **`thesis`** eq. 4.2 — *"α is a parameter regulating how quickly the formula adapts to new information. In our experiments, α is always kept on 0.7"* (lines 3809-3810); **the 0 default is `measured`**: the [#185 A/B](https://github.com/danieljoppi/AntHocNet/issues/185#issuecomment-5823817986) (10 paired seeds, disk + two-ray, AODV byte-identical) read PAIRED-NOISE for 0.7 on both channels, so it stays off. This — not `alpha` — is the thesis's α. | Smooths the hop count per (destination, next hop) before it enters the pheromone metric: `ĥ ← α·ĥ + (1−α)·h_ant`, seeded by the first ant and re-seeded when the route lapses. `0` = each ant's own hop count (pre-#185, byte-identical); `0.7` = the thesis. Only the hop term is smoothed; the time term's smoothing is the γ running average on the whole deposit. See [`fidelity.md`](fidelity.md) deviation 9. |
| `betaAnts` | `20.0` | exponent | `BetaAnts` | **Thesis β₁ = 20** (reactive ants, eq 4.1 "we keep β₁ relatively high, on 20") and **β₂ = 20** (proactive ants, eq 4.5 "normally kept on 20") — two exponents this one field conflates, harmless only because they coincide. Adopted on a 20-seed × 900 s paired A/B: 5 of 6 scenarios PAIRED-IMPROVED, none worse ([#179](https://github.com/danieljoppi/AntHocNet/issues/179)). Departs from `[1] §3.3`'s *unsquared* ants — see [fidelity.md](fidelity.md). | Exploration greediness of ants. Higher concentrates ants on known-good hops (less exploration). |
| `betaData` | `20.0` | exponent | `BetaData` | **Thesis β₃ = 20** (data, eq 4.6 "we normally keep β₃ on 20"). Adopted on the same paired A/B as `betaAnts`; the largest single effect was `heavy-load` delay99 −192.50 ms [−210.65, −174.40] ([#179](https://github.com/danieljoppi/AntHocNet/issues/179)). Departs from `[1] §3.2`'s pheromone-**squared** data rule — see [fidelity.md](fidelity.md). | Greediness of stochastic data forwarding. Higher = closer to single-path; lower = more spread (and more reordering). |
...
| `proactiveVirtualMargin` | `0.0` | fraction | `ProactiveVirtualMargin` | Mechanism **`thesis`** (lines 4084-4088); **shape is `measured` — per-link, not the thesis's best-vs-best** ([ADR-0018](adr/0018-emission-gate-compares-per-link.md)); **the 0 default is `repo choice`** — see [`fidelity.md`](fidelity.md) deviation 4b. | Fraction by which some neighbour's *virtual* pheromone must exceed the *regular* pheromone **on that same link** before a considered proactive ant is emitted (a hint on an unsampled link passes trivially). `0` = gate off, emit every tick. The thesis's literal best-vs-best form has a structural ceiling ≈ h/(h−1), so any fixed margin silently disabled the gate beyond 1+1/m hops — measured on `exp_uniformity_probe` after the #262/#279 table fixes; the historical 0.10 damage (pass rate 0-3.8%, NRL +36-68%, PDR −5 to −6.4 pp) is PR #188's record. Per-link, the probe puts the meaningful range at **m ≈ 0.2-0.5**; adopting a non-zero default was tested on the MANET taxonomy by [#180](https://github.com/danieljoppi/AntHocNet/issues/180) and **rejected**: at m=0.3 with the 2 s rate, 5 of 6 scenarios are PAIRED-WORSE and NRL rises on 6 of 6, so the gate stays default-off and regime-conditional — the 2026-08-02 satellite-corridor win (m=0.3, −35 % proactive traffic) keeps its own measured basis and does **not** generalise to contended wireless fields. |
...
| `proactiveInterval` | `10.0` | s | `ProactiveInterval` | **`measured — #248 sweep`**: 7-point curve (1/2/5/10/20/50 s/off, paper field, 900 s, paired seeds) is *strictly monotone toward less proactive* — the thesis's 2 s optimum inverts on our field (2 s: −15.8 pp PDR, ×5.3 NRL vs 10 s; congestion collapse at 1 s), and `EnableProactive=false` is the measured MANET optimum (97.2 PDR / 31.73 NRL vs 94.9/40.51 at 10 s). 10 s is retained as the cross-regime default: no rate wins on the MANET field (the curve's limit is "off"), and the satellite regime (#192) needs proactive sampling — regime-conditional settings ride the build/configuration axis (#206), not this default. MANET benchmark guidance: `EnableProactive=false`. **Re-confirmed on the β=20 baseline by the [#180](https://github.com/danieljoppi/AntHocNet/issues/180) campaign** (2 s + gate m=0.3, 6 scenarios × 20 seeds × 900 s, paired): PDR down on 5 of 6 (to −12.5 pp at `large-scale`), delay99 up on 5 of 6 (to +519 ms), NRL up on **6 of 6** — the mirror image of #179, where it fell 5/5. | Proactive forward-ant rate per active session. Lower = faster path re-optimisation, more overhead — measured net-negative at every rate on the paper MANET field (#248). |
...
| `proactiveInterval` | `10.0` s | **Resolved by measurement (#248, re-confirmed #180).** The thesis states `t_hello` = 1 s with a 2 s optimum, so 10 s is a deliberate deviation rather than an oversight: the thesis rate was measured on this harness twice — once at β=1/2 (#248) and once at β=20 with the ADR-0018 gate (#180) — and lost decisively both times. No longer a provenance gap. | Resolved. |
...
| `MaxReinjectPerPacket` | `0` (unlimited) | Issue [#386](https://github.com/danieljoppi/AntHocNet/issues/386) remedy: caps how many times a single data packet may be re-injected by the #46 MAC-failure detector across its whole path (a packet tag carries the count between hops — ns-3 metadata, never on the wire). Motivation: packets are re-injected ~2.1–2.3 times on average, and most re-injections are of already-delivered packets, while the mechanism as a whole rescues several pp of PDR under fading — the cap aims to keep the rescue and cut the waste. **The v1.5.0 sweep has now run** (cap ∈ {1, 2, ∞} × detector-off arms, 20 seeds, both fading mobilities — [results](benchmarks/reinjection.md)): at 20 seeds the duplicate rate is 65–67 % and the detector's own win is +5.55/+6.54 pp. Neither pre-registered auto-criterion fired, so the operating point is a maintainer choice and **the default stays `0` = unlimited**, which also preserves the historical behaviour behind every published number and leaves the cap logic unreachable at that default. The measured cost of capping: every cap arm loses PDR (cap=1 −1.89/−1.40 pp, cap=2 −0.44/−0.91 pp) because it converts re-injections that were delivering into `pktsNever` (2.66 % → 13.41 % at cap=1), while the ~48 % cut in re-injected transmissions buys only −2.4/−4.8 % NRL. Set `2` if you want the transmission saving at the smallest measured delivery cost. Semantics caveat measured in the [#402](https://github.com/danieljoppi/AntHocNet/issues/402) diagnosis: the budget is enforced **per packet copy**, not per `(flow, seq)` key — duplicate copies at different nodes each carry their own `ReinjCountTag`, so a heavily-duplicated packet can be re-injected more than `cap` times in total (`##REINJ##` `skips` > `skipsPkts` under cap=1: 387 events vs 370 keys). |
| `QueueTimeout` | `3 s` | How long a data packet may wait for a route before being dropped (#21 deliver-late vs discard trade). |
| `ReconvHoldCap` | `200 ms` | Caps how long a reconvergence-held packet may wait (issue #21 lever L2, #103/#104). Bounded by `QueueTimeout`. **This is an operating-point choice, not a neutral default**: the [#308 ablation](https://github.com/danieljoppi/AntHocNet/issues/308#issuecomment-5211529535) showed causally that the hold converts would-be drops into late deliveries, so the cap trades the `delay99` tail against the PDR advantage — and the [#309 sweep](https://github.com/danieljoppi/AntHocNet/issues/309) found `200 ms` beats AODV on every published metric with a far smaller tail. The default is `200 ms` since the [#371](https://github.com/danieljoppi/AntHocNet/issues/371) flip, landed for the [v1.5.0 re-baseline campaign](benchmarks/v1.5.0-campaign.md); `1 s` was the shipped value from v1.3.0 through v1.4.0 — delivery-biased and off the measured efficient frontier. Every number published before the v1.5.0 re-baseline was measured at 1 s; set `1 s` if your workload prices the last points of PDR above the tail. |
...
### Pheromone & goodness — `gamma`, `hopCountAlpha`, `betaAnts`, `betaData`, `hopTimeSec`, `enableMacMetric`, `alpha`, `enableEvaporation`, `evaporationInterval`, `minPheromone`
...
reorders packets), raising it approaches deterministic best-path. At the shipped
20 the data flow is close to deterministic best-path already; `betaAnts` is the
knob to lower if you want more exploration, since ants and data no longer differ.
