...
| `proactiveVirtualMargin` | `0.0` | fraction | `ProactiveVirtualMargin` | Mechanism **`thesis`** (lines 4084-4088): *"only if the best virtual pheromone is significantly better (in our experiments: at least 10% better) than the best regular pheromone, a proactive forward ant is sent out"*. **The default is `repo choice` and deliberately NOT the thesis's 0.10** — see below and [`fidelity.md`](fidelity.md) deviation 4b. | Fraction by which the best *virtual* pheromone must exceed the best *regular* pheromone before a considered proactive ant is actually emitted. `0` = gate off, emit every tick. **Shipped off because 0.10 was measured harmful** ([#180](https://github.com/danieljoppi/AntHocNet/issues/180)): it suppresses ~all maintenance (1.9% pass rate on a 4×4 grid, 0% on a line) because it compares a bootstrapped estimate against a measured one, and the resulting stale routes triple reactive rediscovery — ns-3 measured NRL +36-68%, PDR −5 to −6.4 pp. Not a threshold to retune; #180 owns re-deriving the *condition*. |
