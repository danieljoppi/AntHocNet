...
flowchart LR
    ...
    V19["<b>v1.9.0</b><br/>family axis II:<br/>VANET on Veins"]
...
flowchart TB
    ...
    E301["<b>#301</b> VANET<br/>#485 Veins coupling"]
...
| **v1.8.0** | **OMNeT++/INET adapter** ([#32](https://github.com/danieljoppi/AntHocNet/issues/32)): ports [#147](https://github.com/danieljoppi/AntHocNet/issues/147), routing module [#148](https://github.com/danieljoppi/AntHocNet/issues/148), example + INET baseline comparison [#149](https://github.com/danieljoppi/AntHocNet/issues/149), toolchain image + CI [#150](https://github.com/danieljoppi/AntHocNet/issues/150). Exit: the adapter builds against a pinned OMNeT++/INET, multi-hop delivery routes end-to-end in CI, and an AntHocNet-vs-baseline run emits the shared CSV. Released on its own because it is the largest single piece of engineering in the v1.x line and VANET cannot start without it. |
| **v1.9.0** | **Family axis II — VANET** ([#301](https://github.com/danieljoppi/AntHocNet/issues/301)) on Veins ([#485](https://github.com/danieljoppi/AntHocNet/issues/485): SUMO/TraCI coupling, 802.11p-class radio, pinned toolchain). Exit: the README family table shows ≥4 supported families (MANET, static mesh, FANET, VANET), each with a results page, plus a cross-family ranking-stability statement — the criterion the old v1.6.0 row carried. |
