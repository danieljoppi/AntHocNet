...
| [#423](https://github.com/danieljoppi/AntHocNet/issues/423) | `##ORACLE##` missing from the compact block; oracle `noRoute` in no drop bucket | item 1 closed by [PR #438](https://github.com/danieljoppi/AntHocNet/pull/438) (re-emit added, and the marker re-emit list is now CI-gated). **Item 2 is now settled, and this row previously mis-stated it** ([evidence](https://github.com/danieljoppi/AntHocNet/issues/423#issuecomment-5455265085)): the `noRoute` packets are neither retried nor mis-bucketed — `RouteOutput` returns `nullptr`, the send fails at the socket layer, and the packet reaches neither the drop book nor the PDR denominator. Measured: **100.0 % PDR on a seed with 4512 refused sends**. It is a validity defect in the arm the delivery decomposition divides by, not a diagnostic, and it does touch a published number — the v1.5.0 grid ran under the 300 m disk and carried `noRoute` of 225/44/25/12/8/2, making its oracle PDR optimistic by ≈0.2 pp. Carried forward as [#464](https://github.com/danieljoppi/AntHocNet/issues/464), **closed 2026-09-06** by [PR #466](https://github.com/danieljoppi/AntHocNet/pull/466) |
| [#464](https://github.com/danieljoppi/AntHocNet/issues/464) | the oracle's refused sends are absent from both the drop book and the PDR denominator | **filed 2026-08-28 out of #423 item 2; closed 2026-09-06** by [PR #466](https://github.com/danieljoppi/AntHocNet/pull/466). The counter could not be used as the issue proposed: `Topology::m_noRoute` conflates `RouteOutput` refusals (never reach `Ipv4L3Protocol::Send`, the actual denominator hole) with `RouteInput` mid-path route losses (already booked in the `route` column), so folding the whole counter in would have double-counted the second population. #466 records the split at the two failure paths, folds **only** the origin half into `txPackets`/`dropRoute`, appends `noRouteOrigin=`/`noRouteFwd=` to `##ORACLE##`, and makes `scenario_check.py` **FAIL** an unbooked cell (135 cases, was 132). Measured on a partitioned cell: seed 4 read `pdr=100.0` alongside 4512 refused sends before, `pdr=37.3` after; fully partitioned now closes at `route=100.00 sum=100.00` instead of `sum=0.00`. Scope against published numbers: **117 of the 120** re-measured grid seeds carry no refusals; three fading seeds carry 2, 2 and 30 failed lookups (#457's PHY-derived radii keep the field connected where the 300 m disk did not, but not everywhere) — an earlier version of this row, and of the issue body, said 120/120 and was wrong ([correction](https://github.com/danieljoppi/AntHocNet/issues/464#issuecomment-5489824861)). Nothing published from the #431 re-measure moves at the precision it is quoted to — 34 refused lookups against ~120 000 offered packets per cell is worth hundredths of a point — and the matched-hop tables are untouched regardless, since a refused packet is delivered by no arm and so can never enter the `##COMMON##` intersection. **One part of the issue did not ship**: the a-priori control the #229/#230 rule requires, which stays with #230 below |
