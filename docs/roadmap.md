...
flowchart LR
    ...
    V18["<b>v1.8.0</b><br/>OMNeT++/INET<br/>adapter"]
    V19["<b>v1.9.0</b><br/>family axis II:<br/>VANET (ns-3 + Veins)"]
...
flowchart TB
    ...
    E32["<b>#32</b> OMNeT++/INET adapter<br/>#147–#150 · the Veins entry ticket"]
    ...
    E32 -->|"Veins confirmation arm only —<br/>the ns-3 arm #488 does not wait"| E301
...
| **v1.7.0** | **Family axis I — FANET + static mesh.** FANET ([#300](https://github.com/danieljoppi/AntHocNet/issues/300)) runs deterministically as `--scenario=fanet`, passes preflight, has its own anchor, and publishes a results page with CIs plus a MANET-vs-FANET ranking-stability statement. Work order: 3D plumbing [#480](https://github.com/danieljoppi/AntHocNet/issues/480) → 3D preflight [#481](https://github.com/danieljoppi/AntHocNet/issues/481) → preset + anchor [#482](https://github.com/danieljoppi/AntHocNet/issues/482) → campaign + page [#483](https://github.com/danieljoppi/AntHocNet/issues/483). The static Wi-Fi mesh family gets its results page from the existing `sparse-static` cell ([#484](https://github.com/danieljoppi/AntHocNet/issues/484), no harness work). FANET goes first because it has the direct literature hook and derisks the mobility plumbing VANET reuses.<br/>**Status (2026-09-26):**<br/>• 3D plumbing ✅: [#480](https://github.com/danieljoppi/AntHocNet/issues/480), merged as [PR #491](https://github.com/danieljoppi/AntHocNet/pull/491). `--areaZ` gives a byte-identical planar default; five 3D refusals; a CI 3-D field smoke that mutation-testing shows catches the flattening bug it was written for.<br/>• 3D preflight ✅: [#481](https://github.com/danieljoppi/AntHocNet/issues/481), merged as [PR #492](https://github.com/danieljoppi/AntHocNet/pull/492). It adds 3D box degree math and the hello-vs-link-lifetime rule, and its first reading matters for #482: the draft envelope (30 nodes, 1000×1000×300 m, 250 m) averages about 3.5 neighbours, at the ln(n) partition floor ([#482 note](https://github.com/danieljoppi/AntHocNet/issues/482#issuecomment-5848545391)).<br/>• Static mesh: a 20-seed run at current defaults is in flight for #484 ([run 36261077793](https://github.com/danieljoppi/AntHocNet/actions/runs/36261077793)).<br/>• Next: the #482 preset (literature provenance + density choice), then the #483 campaign. |
| **v1.8.0** | **OMNeT++/INET adapter** ([#32](https://github.com/danieljoppi/AntHocNet/issues/32)): ports [#147](https://github.com/danieljoppi/AntHocNet/issues/147), routing module [#148](https://github.com/danieljoppi/AntHocNet/issues/148), example + INET baseline comparison [#149](https://github.com/danieljoppi/AntHocNet/issues/149), toolchain image + CI [#150](https://github.com/danieljoppi/AntHocNet/issues/150). Exit: the adapter builds against a pinned OMNeT++/INET, multi-hop delivery routes end-to-end in CI, and an AntHocNet-vs-baseline run emits the shared CSV. Released on its own because it is the largest single piece of engineering in the v1.x line and the Veins arm of VANET cannot start without it (the measured ns-3 VANET arm, #488, does not depend on it). |
| **v1.9.0** | **Family axis II — VANET** ([#301](https://github.com/danieljoppi/AntHocNet/issues/301)), in two arms (maintainer decision 2026-09-26). **Measured arm:** ns-3 Manhattan grid ([#488](https://github.com/danieljoppi/AntHocNet/issues/488)): BonnMotion traces via `Ns2MobilityHelper`, `--scenario=vanet`, its own anchor, a results page with CIs. It carries the numbers because the cross-family statement is only valid inside one simulator: [ADR-0019](adr/0019-network-families-change-the-evaluation-not-the-protocol.md) holds the protocol fixed so a ranking change is the network's, and that fails if the MAC/PHY/simulator changes too ([cross-validation.md](cross-validation.md): cross-simulator is qualitative). It does not depend on #32 and can start once #480/#481 land. **Confirmation arm:** Veins ([#485](https://github.com/danieljoppi/AntHocNet/issues/485): SUMO/TraCI, 802.11p-class radio, pinned toolchain) reports whether the ns-3 ranking holds qualitatively on the stack VANET reviewers expect. Exit: the README family table shows ≥4 supported families (MANET, static mesh, FANET, VANET), each with a results page, plus a cross-family ranking-stability statement on ns-3 — the criterion the old v1.6.0 row carried. |
...
### Why OMNeT++ (#32) is scheduled at v1.8.0, not v2.0.0
...
to defend an architectural claim — it is the entry ticket to the VANET
audience: the Veins confirmation arm of
[#301](https://github.com/danieljoppi/AntHocNet/issues/301) ([#485](https://github.com/danieljoppi/AntHocNet/issues/485)).
It is *not* what makes VANET measurable — the measured arm is ns-3
([#488](https://github.com/danieljoppi/AntHocNet/issues/488)), because a
cross-family comparison is only valid inside one simulator. That is why it
is scheduled with the family axis — at **v1.8.0**, between FANET (v1.7.0) and
VANET (v1.9.0) — rather than alongside the NS-2 removal at v2.0.0. It was
originally slotted at v1.6.0 together with both families; the three were split
into separate releases on 2026-09-26 so each can ship when it is done rather
than all waiting on the slowest (the adapter).
