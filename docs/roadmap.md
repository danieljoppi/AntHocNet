...
| [#431](https://github.com/danieljoppi/AntHocNet/issues/431) | the oracle is exact only on wired topologies | **reopened 2026-08-18, re-measure in flight.** [PR #457](https://github.com/danieljoppi/AntHocNet/pull/457) derived the fading adjacency from the installed PHY decode threshold, but the re-measure that was to accept it failed the hop gate in all six cells — and auditing that failure found the cause was not the oracle. The harness drew flow start times from an arm-dependent RNG stream, so `##COMMON##`'s `(flow, seq)` keys named packets *sent at different times in each arm*; the identity-matched instrument was matching by index, not by transmission ([PR #459](https://github.com/danieljoppi/AntHocNet/pull/459), merged 2026-08-19). The six cells are re-dispatched on the fixed harness with the reading [pre-registered](https://github.com/danieljoppi/AntHocNet/issues/431#issuecomment-5336828819) before the numbers land. The delivery bound is a per-arm statement and is untouched throughout; what is unresolved is whether the published hop defect was the instrument, a real `approx=1` effect, or both |
| [#460](https://github.com/danieljoppi/AntHocNet/issues/460) | every published `##COMMON##` identity-matched number was measured on a time-scrambled matched set | **filed 2026-08-19**, the blast radius of the #459 defect, split out so the fix and the affected claims are tracked apart. Six locations inventoried: `grid.md`'s matched-hop and matched-latency tables, the `methodology.md` paragraph reasoning from them, the `metrics.md` rule, the index row, and the ICNS3 paper's identity-matched row. Per-arm statistics — PDR, delay, delay99 as reported, NRL, the delivery decomposition — are unaffected, as is the satellite suite, which never used the instrument. Blocked on the #431 re-dispatch for the numbers to restate |
