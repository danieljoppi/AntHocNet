...
| **FANET** | 3D smooth trajectories (Gauss-Markov standard), 10–30 m/s, sparse | third dimension, faster link churn, energy budgets that matter | **in progress (v1.7.0, [#300](https://github.com/danieljoppi/AntHocNet/issues/300)).** The harness places and moves nodes in a 3-D box (`--areaZ`, [#480](https://github.com/danieljoppi/AntHocNet/issues/480)) and preflight checks 3-D cells ([#481](https://github.com/danieljoppi/AntHocNet/issues/481)). Still pending: the sourced `--scenario=fanet` preset and anchor ([#482](https://github.com/danieljoppi/AntHocNet/issues/482)), then the 20-seed campaign and results page ([#483](https://github.com/danieljoppi/AntHocNet/issues/483)); until then 3-D runs are harness validation only ([methodology](benchmarks/methodology.md#3-d-fields---areaz-480)). Priority rationale: 2024–2026 FANET surveys evaluate AntHocNet directly and rate it strongest among the classical protocols they test — the family where the protocol's reputation is currently made |
| **VANET** | road-constrained (Manhattan, SUMO traces), 10–40 m/s, platooning | churn is fast but street-shaped; density swings block-by-block; RSU/infrastructure hybrids common | planned for v1.9.0 ([#301](https://github.com/danieljoppi/AntHocNet/issues/301)): an ns-3 Manhattan-grid arm carries the measured numbers ([#488](https://github.com/danieljoppi/AntHocNet/issues/488)), and a Veins/SUMO arm confirms the ranking qualitatively ([#485](https://github.com/danieljoppi/AntHocNet/issues/485)) |
...
| Pending-queue timing (`QueueTimeout` 3 s, `ReconvHoldCap` 200 ms, `RepairHoldCap` 0, `ReactiveRetryInterval` 0.25 s) | #21/#103 frontier values; `ReconvHoldCap` re-picked at 200 ms by [#371](https://github.com/danieljoppi/AntHocNet/issues/371) | live — tuned on this regime's delay tail | live but calibrated against 802.11 contention delays, not 5–18 ms propagation floors ([#205](https://github.com/danieljoppi/AntHocNet/issues/205)) |
