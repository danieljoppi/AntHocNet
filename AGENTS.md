...
| **Look up a parameter's default, its provenance, or how to calibrate it** | [`docs/configuration.md`](docs/configuration.md) — all 37 `Config` fields with a `source` column ([1] §/thesis/repo choice/**unknown**), the ns-3 attribute + NS-2 bind for each, the sweep→A/B→noise loop, and the checklist for adding a parameter. The `unknown` rows in §3.2 are the next #88/#169 candidates |
